...
/** Groq's API is OpenAI-compatible (chat/completions) — fast inference, generous free tier. */
export class GroqProvider implements LlmProvider {
  ...
  async verify(): Promise<VerificationResult> {
    ...
    try {
      ...
      return { ok: true, message: `Key accepted by Groq (${this.model}).` };
    } catch (err) {
  ...
  private async request(req: CompletionRequest): Promise<string> {
    ...
    try {
      ...
      throw toNetworkError(err, 'Groq', this.baseUrl);
    }

    if (!res.ok) {
      const body = await safeText(res);
      throw new ProviderError(describeHttpFailure('Groq', res.status, body), res.status);
    }
